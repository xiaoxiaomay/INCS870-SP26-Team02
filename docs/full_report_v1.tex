...
\begin{table}[!ht]
    ...
    \midrule
    OWASP Agentic Top~10~\cite{owasp_agentic_promptfoo}
      & Agent-specific threat taxonomy
      & Addresses mechanism (privilege, tools) not content semantics \\
    \bottomrule
    \end{tabular}}
\end{table}
...
\node[gate, below=of g0b] (g1) {Gate 1: Embedding\\Precheck (Dual-$\tau$)};
...
\begin{table}[!ht]
    ...
    \midrule
    Indirect Injection & Hidden instructions in retrieved documents & LLM01/03 & Grounding validation \\
    \midrule
...
\For{each sentence $s_i$ in sentences}
    ...
    \ElsIf{$\text{score}_i \geq \tau_s$}
        ...
        \If{$\text{consecutive\_soft} \geq k$}
            \State Mark current and prior soft-hit sentences as \textsc{Redact}
        \Else
...
Grounding validation serves two purposes. First, it mitigates \textit{indirect injection} attacks (OWASP LLM01/LLM03) where poisoned documents in the retrieval corpus instruct the LLM to disclose secrets. Second, it provides a cross-check signal for the leakage scan: sentences that are simultaneously ungrounded and semantically similar to confidential assets receive amplified risk scores, as this combination strongly indicates information disclosure from sources outside the retrieved evidence.
...
\begin{table}[!ht]
    ...
    \midrule
    L2 & Confidential & ``Our desk uses RSI-based mean reversion signals with volume confirmation.'' & Warn \\
    \midrule
...
Several limitations should be noted. First, the evaluation corpus is synthetic: the 60 confidential secrets are hand-authored to be realistic but do not represent actual proprietary strategies. Real-world deployment would use genuine proprietary data, which may exhibit different semantic characteristics. Second, the evaluation set (70 attacks, 20 spectrum entries) is relatively small; a larger and more diverse attack corpus would provide stronger confidence in the results. Third, the embedding model (all-MiniLM-L6-v2) is a general-purpose model; domain-specific embeddings (e.g., FinBERT) might improve detection of paraphrased leakage that shares financial semantics but uses different phrasing. Fourth, the prompt distribution monitoring (C4) is implemented but not enabled in the production configuration; its impact on detection rates is not evaluated in this study. Finally, tool governance (OWASP LLM06---excessive agency) remains as future work; SentinelFlow currently addresses the RAG+LLM path but does not inspect agent-to-tool interactions.
