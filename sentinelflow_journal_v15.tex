...
British Columbia, BC V5M 4X3, \{xwu42, zzhang82, qfang01\}@nyit.edu}
}

%%%
\markboth{Measuring Public/Proprietary Boundary Leakage in Financial RAG}
{Wu \MakeLowercase{\textit{et al.}}: Measuring Public/Proprietary Boundary Leakage}
\maketitle
...
strategies share vocabulary with public financial knowledge, so exact-match DLP
cannot separate them. This paper is a measurement study of that boundary. We build a public/proprietary testbed of 90
...
FPR${=}0$), and the limit is a property of the mechanism class, not of one
encoder. (ii)~Output-layer disclosure detection lies on an empirical
recall--precision frontier that no evaluated mechanism crosses. Literal, regex,
embedding-similarity, document-classifier, and LLM self-check each achieve
either high recall at a precision near that of near-universal flagging
($\approx 0.60$) or usable precision (0.74) at recall 0.50. (iii)~Defensive
value is almost entirely query-side: end-to-end judged disclosure falls from
57.8\% to 34.4\% with the full pipeline, a reduction the pre-LLM gate carries
($+24.4$~pp) while the post-LLM scan contributes within noise ($-1.1$~pp).
(iv)~Disclosure is structured by attack mechanism and secret type:
parameter-targeting attacks against parameter-substitution secrets disclose at
0.94, whereas full-configuration and encoded-export routes disclose at or below
0.14. (v)~All findings use a non-circular protocol, with disclosure judged by an
independent LLM blind to the system's similarity score, and we disclose the
measurement's own boundaries, including a false-positive rate that rises with
proximity to the protected content (1.0\% on benign traffic to 18.9\% on
knowledge-boundary anchors). Semantic boundary leakage is thus measurable and
partially mitigable, but on this testbed remains difficult to separate reliably
from public knowledge sharing the same semantic neighborhood; current
similarity-based and output-layer detection mechanisms do not overcome the
structural limits we measure.
\end{abstract}
...
This section surveys existing work across the areas most relevant to this study: security risk taxonomies, semantic gateways, distance-based data loss prevention, enterprise LLM data-leakage guardrails, behavioral defense, adversarial robustness testing, and agentic security. Each subsection identifies the capabilities and limitations of current approaches, culminating in a gap analysis that motivates the measurement questions this paper investigates.
...
The \textbf{NIST AI Risk Management Framework (AI RMF)} and its companion \textbf{NIST AI 600-1 Generative AI Profile}~\cite{nist_ai_rmf, nist_ai_600_1} emphasize governance, risk measurement, accountability, and auditability as requirements for AI systems in regulated settings. These frameworks motivate the instrument's audit evidence chain, which provides tamper-evident, replayable decision logs for compliance-ready deployments; it supports reproducibility and forensic traceability but is not claimed as a standalone contribution. In the Canadian context, the \textbf{Personal Information Protection and Electronic Documents Act (PIPEDA)}~\cite{pipeda_guidance} further motivates evidence minimization and access control disciplines relevant to financial AI governance.

\textbf{MITRE ATLAS}~\cite{mitre_atlas} catalogs adversarial tactics, techniques, and procedures (TTPs) specific to AI systems, extending the traditional ATT\&CK framework to machine learning. ATLAS provides structured threat intelligence for attacks including model evasion, data poisoning, and inference-time manipulation: informing the adversarial evaluation methodology used to measure boundary-detection behavior in this study.
...
Traditional Data Loss Prevention (DLP) systems rely on exact-match patterns (regular expressions for credit card numbers, Social Security numbers, etc.) and keyword-based rules. These approaches are poorly suited to financial strategy leakage because confidential strategies are \textit{semantic} in nature: they consist of combinations of concepts, thresholds, and conditions that are individually public but collectively proprietary.

This also distinguishes the present problem from the broad literature on sensitive-data and PII detection. PII and contextual-integrity detectors target content whose sensitivity is intrinsic to a token or span (an identifier, a name, a medical code) and that does not appear in public reference material in the same form. The leakage studied here is the opposite case: every token in a disclosed proprietary rule (``14-day RSI,'' ``below 30,'' ``2x average volume'') also appears in public practitioner knowledge, and what is confidential is the specific \textit{configuration}, namely which parameter, which threshold, which combination. The discrimination target is therefore not ``is this a sensitive entity?'' but ``does this output reconstruct a proprietary specialization of a public concept?'', a public/proprietary \textit{boundary} problem rather than an entity-recognition problem, which is why neither pattern-based DLP nor PII/sensitive-data classifiers transfer to it directly.
...
The instrument evaluated in this study uses SBERT-based cosine similarity as its core detection mechanism (the all-MiniLM-L6-v2 model, 384-dimensional embeddings) to encode both queries and LLM responses, and extends the basic similarity approach in two ways relevant to what it can measure. First, it operates at \textit{sentence-level granularity}: rather than computing a single similarity score for the entire response, it splits the LLM output into individual sentences and evaluates each against the secret index independently, which makes partial leakage, where only one sentence reveals confidential parameters, observable. Second, it implements \textit{cascade logic}: when multiple consecutive sentences exceed a soft similarity threshold, a cascade trigger fires even if no individual sentence reaches the hard threshold, surfacing partial leakage spread across sentences \textit{within a single response}. The related cross-query, session-level salami pattern is discussed in Section~\ref{Sect:SalamiDetector} and is not evaluated here.

\subsection{Enterprise LLM Data-Leakage Guardrails}
\label{Sect:LR_Enterprise}

Two recent systems target the same high-level problem as this study, preventing proprietary or sensitive enterprise data from leaking through LLM workflows, and are therefore the closest points of comparison.

\textbf{SecMulti-RAG}~\cite{secmulti_rag} augments enterprise RAG with multiple knowledge sources (internal documents, pre-generated expert knowledge, and on-demand external-LLM knowledge) and a confidentiality-preserving filter that decides whether a query is safe to forward to a closed-source external LLM, routing sensitive prompts to a local open-source generator instead. Its security mechanism is thus an \textit{input-side} routing decision, whether a prompt should leave the trust boundary at all, demonstrated on an automotive report-generation task. The instrument used here differs on both the control point and the discrimination target: it inspects the \textit{outbound response} at the public/proprietary boundary rather than gating outbound prompts, and the problem it is built to probe is separating proprietary parameters from public knowledge that shares the same vocabulary (``RSI below 30'' versus a fund-specific entry rule), a distinction that a safe/unsafe prompt router does not draw. It additionally implements sentence-level cascade detection of partial within-response leakage and a tamper-evident audit chain, and this study adds a verbatim-grounded isolation-failure measurement (Section~\ref{Sect:PathA}), none of which are components of SecMulti-RAG.

\textbf{SafeGPT}~\cite{safegpt} is the closest prior system in shape: a two-sided guardrail combining input-side detection and redaction, output-side moderation and reframing, and human-in-the-loop feedback. Its output-side stage targets generic enterprise sensitive data and policy-violating content, drawing on toxicity classifiers and pattern-based DLP for structured identifiers. The instrument used in this study probes a different discrimination target: rather than moderating toxicity or rewriting content, it scores each response sentence for semantic proximity to a registry of confidential financial strategies, using intent-aware dual thresholds calibrated to the public/proprietary boundary. Where SafeGPT relies on human review as its backstop, this study instead quantifies, against an independent verbatim ground truth (Section~\ref{Sect:PathA}), where similarity-based detection succeeds and where it must be supplemented by an explicit parameter-presence check. The encoder-as-defense-lever analysis (Section~\ref{Sect:V_A_Phase1F}) measures a design axis (the choice of embedding model) that the SafeGPT design does not isolate.
...
While these works address important dimensions of agentic security (privilege escalation, tool misuse, goal hijacking), they focus on the \textit{mechanism} of exploitation rather than the \textit{content semantics} of what information is disclosed. This study probes a different point in that space: output-side semantic inspection at the user-facing boundary, where (regardless of whether the response was shaped by cross-agent delegation, knowledge-graph traversal, or direct retrieval) the content is evaluated before it reaches the user. This does not protect intermediate agent-to-agent communications; it is a last-line content check at the point where information exits the system.
...
\begin{table*}[!ht]
    \centering
    \caption{Prior work and the public/proprietary boundary-measurement gap}
    \label{tab:lit_gap}
    \scalebox{0.88}{
    \begin{tabular}{p{3.2cm} p{3.8cm} p{5.5cm}}
    \toprule
    \textbf{Approach} & \textbf{Capability} & \textbf{Gap relevant to this measurement} \\
    \midrule
    Llama Guard, Prompt Shields~\cite{llama_guard, ms_prompt_shields}
      & Intent classification against safety taxonomies
      & Not designed for domain-specific strategy leakage that appears benign in form \\
    \midrule
...
The analysis points to five capability gaps that existing solutions address only partially or in isolation rather than together: (1)~\textit{inline enforcement} with real-time block/redact capabilities positioned before responses reach users, as opposed to post-hoc monitoring and alerting; (2)~\textit{domain-specific semantic leakage detection} that can distinguish between public financial knowledge and proprietary strategy parameters sharing the same vocabulary; (3)~\textit{sentence-level granularity} with cascade logic for detecting gradual exfiltration across multiple sentences; (4)~\textit{tamper-evident audit chains} providing replayable, hash-linked evidence for compliance in regulated environments; and (5)~a \textit{domain-specific adversarial evaluation framework} for systematically testing financial strategy leakage defenses. This study uses SentinelFlow as the instrument to measure where, at the public/proprietary boundary, a defense built around these capabilities succeeds and where it structurally fails.

%%% Section starts here
\section{Proposed Solution}
\label{Sect:Proposed_Solution}

This section describes the instrument used to generate the measurements in this paper; its components are not claimed as standalone security contributions. The design choices documented below matter because they determine what the instrument can and cannot measure, not because any individual gate is offered as a novel defense. Unlike post-hoc monitoring tools that detect leakage after it occurs, SentinelFlow enforces security decisions in real time: blocking unsafe queries \textit{before} the LLM is invoked and redacting flagged content from responses \textit{before} they reach the user. The system comprises a multi-gate pre-LLM query filter, a post-LLM sentence-level leakage scan, and a tamper-evident audit chain; the design choices described below (the dual-threshold gate, the leakage-scan cascade, and the secret-index isolation) are what make the measurements in Section~\ref{Sect:Experimental_Procedure_Performance_Evaluation} possible.
...
The fail-safe ordering provides two advantages. First, blocked queries never invoke the LLM API, incurring only local regex and embedding-search cost (latency reported in Section~\ref{Sect:Latency}). Second, a query that passes all pre-gates but produces a leaking response may still be caught by the post-LLM semantic firewall before the user sees the output. This layered ordering provides defense-in-depth in the evaluated configuration; as the attribution and frontier results below show, its protective value is concentrated in the pre-LLM gate rather than the post-LLM scan, and it is not a guarantee that disclosure cannot occur. Both the web interface (\texttt{SentinelEngine} class) and the CLI evaluation pipeline implement identical gate logic and thresholds, so the results in Section~\ref{Sect:Experimental_Procedure_Performance_Evaluation} reflect the deployed application's security posture.

\subsection{Threat Model}
\label{Sect:Threat_Model}

SentinelFlow assumes a deployment where analysts query an internal knowledge base through a RAG-augmented LLM. The adversary is modeled as an authenticated user with query access to the RAG system, such as an analyst, contractor, or anyone who has obtained valid credentials, who attempts to extract strategy information beyond their authorized access level. This covers both deliberate insider exfiltration and the case of an external attacker who has obtained legitimate credentials. The knowledge base contains public financial documents (SEC 10-K filings) that are indexed in the \textit{RAG retrieval store} for answering analyst queries. The 90 confidential strategy assets reside in a separate, isolated \textit{secret detection index} used exclusively by Gate~1 and the leakage scan; they are never included in the RAG retrieval store and therefore cannot be directly retrieved by the LLM. This architectural separation is the key reason why queries that bypass the pre-gate layer (50.18\%) do not generally result in secret disclosure: the LLM answers from the public corpus only, while Gate~1 detects semantic proximity to secrets in a parallel index. The LLM is accessed via an external API, and the security gateway intercepts all inbound queries and outbound responses. We emphasize that the high-value scenario for an inline gateway is not this isolated default, where leakage is near-zero by construction, but \textit{isolation failure}: misconfiguration, multi-tenant hybrid retrieval, or retrieval poisoning that causes confidential material to become retrievable. Documented failure modes such as indirect prompt injection, retrieval-corpus poisoning, and vector/embedding weaknesses show that retrieval-time access controls can fail in practice~\cite{owasp_llm_2025}. SentinelFlow is positioned as a defense-in-depth layer for exactly this case, and Section~\ref{Sect:PathA} evaluates it under an explicit isolation-failure scenario.
...
Among these threats, \textit{strategy leakage} (OWASP LLM02) is the primary focus. As established in Section~\ref{Sect:Introduction}, financial strategy leakage is semantic rather than fixed-string: the confidential content is a parameterized specialization of public concepts (the RSI example of \S\ref{Sect:Introduction}), which is why traditional pattern-based DLP cannot draw the public/proprietary distinction and embedding-based detection is used as the measurement instrument here.


\subsection{Gate 0a: Intent Precheck (Regex Rules)}
\label{Sect:Gate0a}

Gate~0a, the first and fastest line of defense, scans the query against compiled regex patterns in five hard-block categories plus one flagging category (hypothetical-framing, which triggers strict threshold enforcement rather than blocking), as shown in Table~\ref{tab:gate0a_rules}.
...
If any pattern matches, the pipeline short-circuits and a generic block message is returned. Gate~0a is deterministic, sub-millisecond, and contributes no false positives (Section~\ref{Sect:Ablation}).

\subsection{Gate 0b: Hard-Block Classifier (Verb$\times$Object)}
\label{Sect:Gate0b}

Gate~0b extends pattern-based detection with a two-part classifier: nine direct regex patterns that block explicit mentions of sensitive system components (e.g., \texttt{system\textbackslash s*prompt}, \texttt{audit\textbackslash s*log}, \texttt{api\textbackslash s*key}), and a \textit{verb$\times$object combinatorial matrix} (13 exfiltration verbs $\times$ 23 sensitive objects) that blocks when a query contains both an exfiltration verb and a sensitive object. For instance, ``export the secret index to JSON'' triggers the verb$\times$object match (\textit{export} $\times$ \textit{secret index}) even though no single direct pattern matches. Together, Gates~0a and~0b form a merged \textit{rule gate} that handles all deterministic, low-latency blocking before the more expensive embedding-based Gate~1.

\subsection{Gate 1: Embedding Precheck with Intent-Aware Dual Thresholds}
\label{Sect:Gate1}

Gate~1 is the first gate that uses semantic understanding: it encodes the query (all-MiniLM-L6-v2, 384-dim~\cite{sbert}) and searches the confidential-assets FAISS index for the top-3 closest secrets by cosine similarity, blocking if the maximum exceeds threshold. Its key device is an \textbf{intent-aware dual-threshold mechanism} that resolves the classic FPR/TPR tension in similarity-based detection.
...
The system detects extraction intent by substring-matching 62 \textit{intent-amplifier} keywords (e.g., \textit{parameters, thresholds, exact, entry/exit conditions, formula, signal, position limits, drawdown, fee structure, leverage, hypothetical, assume}); any amplifier match triggers the strict threshold $\tau_e = 0.50$ regardless of surrounding context. A supplementary hypothetical-framing rule set identifies academic and roleplay framing (e.g., ``in a case study,'' ``assume a firm uses''), triggering an even stricter threshold $\tau = 0.45$ for that subset.
...
The leakage scan is a post-LLM semantic firewall that inspects the generated response at sentence-level granularity before it reaches the user: a final defense layer even when a query passes all pre-gates.

\subsubsection{Algorithm}

The scan splits the LLM response into sentences (regex split on \texttt{.!?} boundaries), encodes each with the same sentence-transformer, and scores it against the confidential-assets FAISS index; each sentence is then classified by the three-tier thresholds of Table~\ref{tab:leakage_tiers}. Algorithm~\ref{alg:leakage_scan} gives the formal pseudocode.
...
Regulated financial environments require tamper-evident audit trails for compliance and incident reconstruction~\cite{nist_ai_rmf}. The instrument implements an append-only JSONL audit log with dual SHA-256 hash chains; this supports reproducibility and forensic traceability but is not claimed as a standalone contribution.

\subsubsection{Dual Hash Chain Design}

Each audit event carries a timestamp, event type, and structured gate-specific body, together with three hashes that establish provenance: an \texttt{event\_hash} (SHA-256 of the event's canonical JSON, excluding the hash fields), a \texttt{prev\_hash} linking the preceding event in the \textit{global} chain across all sessions, and a \texttt{session\_prev\_hash} linking the preceding event in the \textit{per-session} chain. Fig.~\ref{fig:hash_chain} illustrates the dual chain across two consecutive sessions.
...
The system logs 16 event types spanning the full pipeline (from \texttt{runtime\_info} through each gate decision, the LLM call, the leakage scan, and \texttt{final\_output}), four of them conditionally. Every gate decision is thus recorded in replayable, tamper-evident form: a standard RAG path produces 10 audit events and a blocked query 3, so the audit trail itself confirms the fail-safe cost property (blocked queries never reach the LLM, avoiding both API cost and inference latency).

\subsubsection{Tamper Detection}

If any record is modified after creation, its recomputed hash no longer matches the \texttt{prev\_hash} stored in the subsequent record, breaking the chain; a verification script validates both global and per-session chains, enabling forensic detection of post-hoc tampering. In evaluation, all hash-chain integrity checks passed with 100\% verification success. The provenance, tamper-evidence, and auditability this provides are what let the measurements reported in Section~\ref{Sect:Experimental_Procedure_Performance_Evaluation} be replayed and independently re-verified; the chain supports reproducibility and forensic traceability but is not claimed as a standalone contribution.

\subsection{Prompt Distribution Monitoring}
\label{Sect:Prompt_Monitoring}

The prompt distribution monitoring module is a lightweight behavioral anomaly signal~\cite{nvidia_morpheus}: it embeds 100 representative analyst queries into a centroid, converts each query's distance to a $z$-score, and on exceeding $\sigma{=}2.0$ tightens the leakage-scan thresholds by $\delta{=}0.05$ rather than blocking. It is \textbf{disabled in all evaluations reported here} (\texttt{enabled: false}), on this study's diverse multi-technique corpus it would add roughly 5\% benign FPR without proportional benefit, and is described only because the deployed configuration exposes it; its absence is why the measurements isolate the gate and scan rather than this signal.

\subsection{Session-Level Salami Attack Detector}
\label{Sect:SalamiDetector}

\textbf{This mechanism is designed but not evaluated in this paper}, and must not be confused with the multi-turn salami \emph{experiment} of Section~\ref{Sect:V_salami} (which measures cumulative multi-turn disclosure behavior). Because cross-query salami sub-queries are individually innocent, each uses generic phrasing and passes Gate~1's permissive threshold, they can only be caught across turns; the detector accumulates per-session similarity to the secret index over a rolling window and, on a sustained pattern, tightens Gate~1's threshold for the rest of the session rather than blocking individual queries. The prompt-level experiments in Section~\ref{Sect:Experimental_Procedure_Performance_Evaluation} operate on independent single queries and do not exercise this session state; it is assessed only as future work.

\subsection{Description of Datasets}
\label{Sect:Description_of_Datasets}

SentinelFlow uses a hybrid dataset strategy designed for reproducibility and source-anchored evaluation of financial strategy leakage scenarios.
...
public content. Each decoy must pass a plausibility test: it must read as a
...
batches), each passing a fixed two-stage review: a domain-expert plausibility
...
\subsubsection{Auxiliary Sensitivity Spectrum (L0--L3)}

A key dataset design decision is the four-level sensitivity spectrum that creates a gradient from fully public to fully confidential knowledge. This auxiliary spectrum supports instrument characterization, evaluating the gateway's ability to distinguish legitimate financial education from confidential strategy disclosure, and is not itself one of the paper's contributions.
...
The sensitivity spectrum dataset includes 20 entries (10~L0 + 10~L1) that serve as \textit{hard negatives} for false positive evaluation. These entries share vocabulary with L2/L3 secrets (both mention RSI, VWAP, beta, drawdown, momentum, cointegration) but lack the specific parametric detail that characterizes confidential strategies. This spectrum and the associated legacy secret set belong to the auxiliary material; they cover six institutional alpha domains and are retained for instrument characterization and the measurement-sensitivity analysis (\S\ref{Sect:PathA}), not for the headline corpus\_v2 findings, whose 90-secret testbed spans 16 subdomains (\S\ref{Sect:Testbed}).

\subsubsection{Auxiliary Legacy Adversarial Prompts}

The auxiliary legacy attack set includes 70 adversarial prompts organized into 10 categories: direct extraction (13), indirect extraction (10), salami attack (9), prompt injection (8), social engineering (6), encoding extraction (5), paraphrase extraction (5), indirect injection (5), hard-block keywords (5), and adversarial exfiltration (4). These are retained for instrument characterization (the B0-vs-B2 comparison, ablations, and threshold sweep), not for the headline findings, which use the 90-prompt corpus\_v2 boundary attack set (\S\ref{Sect:V_primary}). Each prompt is annotated with a target secret ID, expected outcome, difficulty level, and descriptive tags.
...
\begin{table*}[!ht]
    \centering
    \caption{Datasets by role. The primary boundary testbed (corpus\_v2) underlies the measurement findings (Sections~\ref{Sect:V_primary}--\ref{Sect:V_salami}); the auxiliary sets support instrument characterization and the sensitivity study.}
    ...
    \midrule
 \multicolumn{4}{l}{\textit{Primary boundary testbed (corpus\_v2): basis of the measurement findings}} \\
    \quad Confidential secrets & 90 & Leakage detection targets (Type A 72 / B 18) & L1--L3 \\
    \quad Anchor hard negatives & 90 & Public/proprietary boundary FPR manifold & L0--L1 \\
    \quad Decoy control set & 37 & Shortcut stress test & L0 \\
 \quad Boundary attack set & 90 & Primary disclosure evaluation (L1-judge) &: \\
    \midrule
 \multicolumn{4}{l}{\textit{Auxiliary sets: instrument characterization and sensitivity study}} \\
    \quad Public corpus (FinDER) & 13{,}867 passages & RAG retrieval index & L0 \\
 \quad Internal attack prompts & 271 (70 + 201 paraphrases) & Bypass / ablation / encoder analyses &: \\
    \quad Sensitivity spectrum & 20 & Threshold-sweep false-positive probe & L0--L1 \\
    \quad Benign prompts & 100 & Benign baseline / centroid & L0--L1 \\
    \quad Isolation-failure corpus & 60 secrets / 144 attacks & Measurement-sensitivity study (\S\ref{Sect:PathA}) & L1--L3 \\
    \bottomrule
    \end{tabular}}
\end{table*}
...
This section presents the experimental design, evaluation methodology, and results. The \emph{primary measurement results} (the boundary-separability benchmark, the recall--precision frontier, the gate-versus-scan defense attribution, the disclosure-route structure, and the multi-turn salami extension) are reported in Sections~\ref{Sect:V_E_BoundaryBench}--\ref{Sect:V_salami} on the corpus\_v2 testbed and are the basis of the paper's contributions. The earlier subsections (B0-vs-B2, sensitivity spectrum, threshold sweep, bypass analysis, ablation, latency, and external probes) characterize the SentinelFlow instrument and auxiliary settings; they are reported first for continuity with the system description above, but they are instrument characterization rather than the headline findings. Concretely, the material is organized in three parts. \emph{Core efficacy} covers the B0-vs-B2 baseline comparison, the L0--L3 sensitivity spectrum, and the threshold sweep. \emph{Robustness, cost, and integrity} covers bypass root-cause analysis, the ablation study, encoding-evasion defense, latency, audit-chain integrity, and a supplementary large-scale external evaluation. \emph{Deeper analyses} comprises the encoder-as-defense-lever ablation (\S\ref{Sect:V_A_Phase1F}), the boundary false-positive characterization (\S\ref{Sect:V_C_Phase1E}), and an isolation-failure defense validation (\S\ref{Sect:PathA}). All evaluation data is drawn from automated scripts and logged in reproducible JSON/CSV reports.

\begin{table}[!t]
\centering
\caption{Reading guide for Section~\ref{Sect:Experimental_Procedure_Performance_Evaluation}: which results are the headline findings and which characterize the instrument.}
\label{tab:reading_guide}
\scalebox{0.92}{
\begin{tabular}{l l}
\toprule
\textbf{Subsections} & \textbf{Role} \\
\midrule
\S\ref{Sect:V_E_BoundaryBench}, \S\ref{Sect:V_E_4_Tier0} & Boundary separability and FPR (primary) \\
\S\ref{Sect:V_primary} & \textbf{Primary measurement findings (frontier, attribution, route)} \\
\S\ref{Sect:V_salami} & Multi-turn salami extension (primary) \\
\S\ref{Sect:PathA} & Measurement-sensitivity validation \\
\S\ref{Sect:V_A_Phase1F}, \S\ref{Sect:V_C_Phase1E} & Encoder lever and hard-negative FPR (primary) \\
\S\ref{Sect:BypassAnalysis}--\S\ref{Sect:ExternalFramework} & Auxiliary instrument characterization \\
\bottomrule
\end{tabular}}
\end{table}
...
This instrument-characterization comparison reports the similarity-proxy Attack Success Rate (ASR) between the unprotected baseline (B0) and the full pipeline (B2) across the 70 auxiliary legacy prompts (the headline disclosure results are the judge-based corpus\_v2 measurements of \S\ref{Sect:V_primary}). Table~\ref{tab:b0_vs_b2} presents the per-category breakdown.
...
To measure what the gateway adds beyond the LLM's built-in safety, the same L0--L3 queries are run against the unprotected LLM (B0 mode, no gateway), and the resulting responses are analyzed for leakage using the semantic similarity detector. Table~\ref{tab:b0_spectrum} presents the per-level leakage rates.
...
The sweep reveals two important characteristics. First, lowering thresholds (e.g., $\tau_h = 0.55$, $\tau_s = 0.45$) increases detection to 47.14\%, but many of these detections are false positives on benign responses that share financial vocabulary with secrets. Second, at thresholds above ($\tau_h = 0.75$, $\tau_s = 0.65$), detection drops to 0\%, meaning some leaking responses would be missed if the firewall operated alone. The pre-gates compensate for this on the evaluated set: they block most attack prompts before the LLM generates any response, yielding 0\% similarity-proxy ASR on this internal corpus even when the firewall's post-LLM thresholds are set conservatively. As the independent-judge results (Section~\ref{Sect:V_primary}) show, this proxy figure overstates protection against paraphrased disclosure; it is reported here as a property of the proxy on this corpus, not as a guarantee.
...
\label{Sect:InstrumentCharacterization}

The preceding sections reported the paper's primary measurement findings about the public--proprietary knowledge boundary: boundary separability, disclosure structure, defense attribution, and the empirical recall--precision frontier. The sections that follow serve a different purpose. Rather than adding measurement findings, they characterize the engineering properties of the SentinelFlow instrument itself (bypass behavior, component ablations, encoding-evasion handling, and latency) in the way that an instrument's resolution, noise profile, and operating range are reported alongside the measurements taken with it. These analyses use the 271-prompt internal corpus and per-component ablations rather than the corpus\_v2 testbed, and are included to document the operating characteristics of the measurement instrument and to support the interpretation of the findings above; they are not themselves the paper's contributions.
...
\midrule
B0 (unprotected)               & None              & 100.0 & n/a$^\ddagger$  & 0.0 & 0.0  \\
No G0a (no regex)              & G0b+G1+LS         & 66.1  & n/a$^\ddagger$  & 2.0 & 10.5 \\
No G0b (no verb$\times$obj)    & G0a+G1+LS         & 53.1  & n/a$^\ddagger$  & 2.0 & 10.2 \\
\rowcolor{babyb}
No G1 (no embedding)           & G0a+G0b+LS        & 75.7  & n/a$^\ddagger$  & 0.0 & 0.1 \\
\arrayrulecolor{black}Single $\tau$ ($\tau{=}0.62$)  & All, single $\tau$ & 72.7 & n/a$^\ddagger$ & 4.0 & 9.8 \\
\rowcolor{teagreen}
\textbf{Full pipeline (B2)}    & \textbf{All}      & \textbf{50.2} & \textbf{2.58} & \textbf{2.0} & \textbf{9.8} \\
\arrayrulecolor{black}\bottomrule
\multicolumn{6}{l}{\footnotesize $^\ddagger$Ablated variants are evaluated at the pre-gate stage only (no full LLM invocation), to isolate} \\
\multicolumn{6}{l}{\footnotesize each gate's filtering contribution at low cost; the similarity-proxy flag rate is therefore measured} \\
\multicolumn{6}{l}{\footnotesize only for the full pipeline (B2). ``n/a'' denotes not-measured-by-design, not missing data.} \\
\end{tabular}}
\end{table*}
...
\toprule
\textbf{Strategy} & \textbf{Mechanism} & \textbf{Target} & \textbf{B0 leak} & \textbf{B2 leak} & \textbf{Salami-only} \\
 & & & \textbf{turn} & \textbf{turn} & \\
\midrule
...
\\[2pt]
{\footnotesize\begin{minipage}{\columnwidth}
``leak turn'' = first turn reaching cumulative disclosure (judge label~$\ge$1). All six chains reach cumulative disclosure (label~2) under both B0 and full pipeline B2. Gate blocks 9/30 turns; delays leak by $\approx$1 turn, prevents no chain.
\end{minipage}}
\end{table}
...
\begin{table}[!t]
    ...
    \toprule
    \textbf{Symbol} & \textbf{Model} & \textbf{Revision} & \textbf{Dim} \\
    \midrule
...
The 60-entry corpus is the original confidential-strategy set; the 90-entry corpus is corpus\_v2, the anchored public/proprietary testbed of Section~\ref{Sect:Testbed} (90 secrets, Type~A/B, with 90 public anchors as hard negatives). FAISS indexes are rebuilt for each encoder $\times$ corpus combination from the same secret source, with per-file SHA-256 build manifests for verification.
...
This axis describes the auxiliary 65-entry hard-negative corpus used for the boundary false-positive characterization, not the primary corpus\_v2 testbed (which spans 16 subdomains; \S\ref{Sect:Testbed}). The second axis is this corpus's six alpha domains (price/volume momentum, event-driven, statistical arbitrage, alternative-data signals, risk-factor neutralization, and ML signal pipelines). Crossing the six categories with the six domains yields a $6 \times 6 = 36$ sub-cell matrix; the 65-entry corpus covers all 36 sub-cells, with non-uniform sub-cell sizes in $[1, 7]$ (the concentration pattern is explained in \S\ref{Sect:V_C_6_Findings}).
...
Second, \emph{external validity}: we are deliberately narrow about what this study claims. We do \emph{not} claim that the specific disclosure rates reported here (e.g., 57.8\% no-defense disclosure, the +24.4~pp gate attribution, the per-family rates) transfer to other domains, to other secret distributions, or to real institutional data; these are properties of this finance testbed under this defender model. Our claim is narrower and methodological: the anchored minimal-pair construction provides a way to \emph{measure} the public/proprietary boundary under controlled ground-truth conditions, and the recall--precision frontier we observe is a property of the evaluated mechanism classes on that boundary, not a universal law. What we offer for reuse is the measurement apparatus: the anchored testbed construction and the non-circular adjudication protocol; rather than a number to be cited out of context. With that scope fixed, we state four boundaries explicitly. (1)~The secret corpus is \emph{synthetic}: source-anchored to citable public statements but not drawn from any real institution's confidential data; it is constructed to admit known ground truth and public release, not to certify fidelity to a specific firm's holdings. (2)~The study is \emph{finance-only}: all 90 secrets and their anchors lie within institutional finance, and we make no cross-domain empirical claim. (3)~The attack set is \emph{researcher-authored}: the 271-prompt internal corpus and the 300 HarmBench-format probes are authored by us, and the one fully independent probe set (779 garak prompts) is general-purpose rather than finance-specific, so it exercises robustness to known jailbreak patterns but provides little finance-domain attack coverage; independent, finance-specific red-teaming would strengthen this. (4)~The recall--precision frontier is \emph{empirical, not universal}: it is a property of the five evaluated mechanism classes on this testbed, not a proven bound over all possible detectors. Several headline rates are also measured on small samples (e.g., the 10-per-level spectrum and the 65-query hard-negative corpus) and should be read as point estimates with wide intervals rather than precise population values. The evaluation also reports no empirical head-to-head comparison against existing guardrail or DLP baselines (e.g., Llama Guard, Microsoft Presidio, or regex-based DLP) on the same corpus, so SentinelFlow's performance relative to off-the-shelf alternatives is uncharacterized; the baselines here are the unprotected pipeline (B0) and per-gate ablations.
...
Cross-domain retargeting (supplying a domain-appropriate secret index and per-category rules for other proprietary-knowledge settings (legal, pharmaceutical, semiconductor)) remains future work; the anchored-minimal-pair construction (\S\ref{Sect:Testbed}) and the hard-negative characterization framework (\S\ref{Sect:V_C_Phase1E}) are domain-agnostic by design, but this paper evaluates only the finance domain and makes no cross-domain empirical claim.
